...
namespace MIPS.Net.InstructionSet
{
    ...
    { 
        ...
        public static byte[] __parse(string instruction, Registers registers)
        {
            ...
            // 2. Verificar se a instrução é do tipo "sgt"
            if (parts[0] != "sgt")
            {
                throw new ArgumentException("Instrução inválida. Esperado 'sgt'.");
            }
            ...
            instructionValue |= 0x00 << 26; // Opcode para sgt (0)
            instructionValue |= rsValue << 21;
            instructionValue |= rtValue << 16;
            instructionValue |= rdValue << 11;
            instructionValue |= 0x29; // Funct para sgt (0x29)
