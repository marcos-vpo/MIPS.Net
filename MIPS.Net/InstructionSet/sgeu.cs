...
namespace MIPS.Net.InstructionSet
{
    public class sgeu
    {
        // Compara dois registradores como números sem sinal e define a flag
        // "Greater Than or Equal" (GE) se o primeiro for maior ou igual ao segundo.
        public static void __call(int instruction, ref Registers registers)
        {
            ...
            // Compara os valores dos registradores rs e rt como números sem sinal
            if ((uint)registers[rs] >= (uint)registers[rt])
            {
        ...
        public static byte[] __parse(string instruction, Registers registers)
        {
            ...
            // 2. Verificar se a instrução é do tipo "sgeu"
            if (parts[0] != "sgeu")
            {
                throw new ArgumentException("Instrução inválida. Esperado 'sgeu'.");
            }
            ...
            instructionValue |= 0x00 << 26; // Opcode para sgeu (0)
            instructionValue |= rsValue << 21;
            instructionValue |= rtValue << 16;
            instructionValue |= rdValue << 11;
            instructionValue |= 0x26; // Funct para sgeu (0x26)
