...
namespace MIPS.Net.InstructionSet
{
    public class lar
    {
        ...
        public static void __call(int instruction, ref Registers registers)
        {
            ...
            RotuleMapping? dataEntryAddr = MIPS_CPU.CurrentProgram.GetRotuleByRelativeAddr(address);
            if (dataEntryAddr == null) return;

            // Armazena o endereço no registrador de destino (rt)
            registers[rt] = dataEntryAddr.AbsoluteAddr;
        }
        ...
        public static byte[] __parse(string instruction, Registers registers)
        {
            ...
            // Remover a vírgula do registrador, caso exista
            rd = rd.TrimEnd(',');
            ...
            instructionValue |= 0x13 << 26;   // Opcode para "lar" (0x13)
            instructionValue |= 0 << 21;      // rs (0)
            instructionValue |= rdValue << 16;  // rd
