...
namespace MIPS.Net.InstructionSet
{
    public class sltu
    {   
        // Compara dois registradores como números sem sinal e define a flag "Less Than" (LT) se o primeiro for menor que o segundo.
        ...
        public static byte[] __parse(string instruction, Registers registers)
        {
            ...
            // 2. Verificar se a instrução é do tipo "sltu"
            if (parts[0] != "sltu")
            {
                throw new ArgumentException("Instrução inválida. Esperado 'sltu'.");
            }
            ...
            instructionValue |= 0x00 << 26; // Opcode para sltu (0)
            instructionValue |= rsValue << 21;
            instructionValue |= rtValue << 16;
            instructionValue |= rdValue << 11;
            instructionValue |= 0x2B; // Funct para sltu (0x2B)
