...
namespace MIPS.Net.InstructionSet
{
    ...
    { 
        ...
        public static byte[] __parse(string instruction, Registers registers)
        {
            ...
            // 2. Verificar se a instrução é do tipo "sgtu"
            if (parts[0] != "sgtu")
            {
                throw new ArgumentException("Instrução inválida. Esperado 'sgtu'.");
            }
            ...
            instructionValue |= 0x00 << 26; // Opcode para sgtu (0)
            instructionValue |= rsValue << 21;
            instructionValue |= rtValue << 16;
            instructionValue |= rdValue << 11;
            instructionValue |= 0x27; // Funct para sgtu (0x27)
