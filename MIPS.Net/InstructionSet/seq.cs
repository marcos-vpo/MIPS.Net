...
namespace MIPS.Net.InstructionSet
{
    ...
    { 
        ...
        public static byte[] __parse(string instruction, Registers registers)
        {
            ...
            // 2. Verificar se a instrução é do tipo "seq"
            if (parts[0] != "seq")
            {
                throw new ArgumentException("Instrução inválida. Esperado 'seq'.");
            }
            ...
            instructionValue |= 0x00 << 26; // Opcode para seq (0)
            instructionValue |= rsValue << 21;
            instructionValue |= rtValue << 16;
            instructionValue |= rdValue << 11;
            instructionValue |= 0x24; // Funct para seq (0x24)
