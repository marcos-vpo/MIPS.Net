...
{
    internal class ProcessTableVirtualHeapManager : ABIManaged
    {

        private List<ProcessTableHeapFragment> _fragments = new List<ProcessTableHeapFragment>();
        private List<ProcessTableHeapObject> _objects = new List<ProcessTableHeapObject>();
        private List<ProcessTableHeapPage> _pages = new List<ProcessTableHeapPage>();

        public void UpdatePages(List<ProcessTableHeapPage> toUpdate)
        {
            ...
            if (newHeapLen > HeapLength)
            {
                ...
                _fragments.Add(new ProcessTableHeapFragment(HeapLength, hl));
        ...
        public void WriteObj(int virtualAddr, mOSObject obj, byte[] objBin)
        {
            ...
            ProcessTableHeapObject hpObj = new ProcessTableHeapObject
            {
            ...
            ProcessTableHeapFragment? fg = _fragments.FirstOrDefault(f => f.VirtualHeapStart == virtualAddr);
            if (fg != null)
            {
        ...
        public void Read(int virtualAddr, ref byte[] data)
        {
            ...
            while (remaining > 0)
            {
                ...
                ProcessTableHeapPage page = _pages.FirstOrDefault(p => p.Order == pageOrder);
                if (page == null)
        ...
        internal int FreeObj(int virtualHeapAddr)
        {
            ...
            while (currentAddr != -1)
            {
                ...
                // cria fragmento
                _fragments.Add(new ProcessTableHeapFragment(currentAddr, free.Length));
            ...
            {
                ProcessTableHeapFragment current = _fragments[i];
                ProcessTableHeapFragment next = _fragments[i + 1];
        ...
        internal int UpdateObj(mOSObject obj, byte[] objNewBin)
        {
            ...
            ProcessTableHeapObject? root = _objects.FirstOrDefault(o => o.VirtualHeapAddr == obj.VirtualAddr);
            if (root == null)
                return 0;

            // 2️⃣ Coleta todas as partes (root + continuations)
            List<ProcessTableHeapObject> parts = new List<ProcessTableHeapObject>();
            ProcessTableHeapObject current = root;
            parts.Add(current);
            ...
            if (written < newSize)
            {
                ...
                ProcessTableHeapObject newPart = new ProcessTableHeapObject
                {
                ...
                ProcessTableHeapObject last = parts.Last();
                ...
                ProcessTableHeapFragment? fg = _fragments.FirstOrDefault(f => f.VirtualHeapStart == last.ContinuationAddr);
                if (fg != null)
                {
                    var fragSize = fg.Length;
